...
//    内容页面展示
    function init(){
        ...
        $this->s->display('template/admin/cehua.html');
    }
                ...
                $this->brr.="
                  <tr>
                    ...
                    <td>
                        ...
                        <button type='button' class='btn btn-primary'><a href='index.php?m=admin&f=cehua&a=change&id={$item['id']}'>修改</a></button>
                    </td>
                 </tr>
                ";
            }
...
//    内容添加功能实现
    function addcontent(){
        ...
        $this->s->display('template/admin/cehuaadd.php');
    }
    function getoption($db,$id=3){
    ...
    function addcheck(){
        ...
        if ($file['size'][0]==0){
            ...
            if($row==1){
                $this->jump('添加成功','index.php?m=admin&f=cehua&a=init');
            }else{
                $this->jump('添加失败','index.php?m=admin&f=cehua&a=init');
            }
            exit;
        }
        ...
        foreach ($brr as $key=>$item){
            if (!in_array($item,$type)){
                $this->jump('请上传图片','index.php?m=admin&f=cehua&a=addcategory');
    ...
    function change(){
        ...
        $this->s->assign('id',$id);
        $this->s->display('template/admin/changecehua.php');
    }
    function addtext(){
        $id=$_REQUEST['id'];
        $name=$_REQUEST['name'];
        ...
        if ($file['size'][0]==0){
            ...
            if($row==1){
                ...
                $this->jump('修改失败','index.php?m=admin&f=cehua&a=init');
            }
            exit;
        }
        $type=['image/jpeg','image/png','image/gif'];
