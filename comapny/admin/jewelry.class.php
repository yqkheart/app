...
//    内容页面展示
    function init(){
        ...
        $this->s->display('template/admin/jewelry.html');
    }
...
//    内容添加功能实现
    function addcontent(){
        ...
        $this->s->display('template/admin/jewelryadd.php');
    }
    function getoption($db,$id=8){
    ...
    function addcheck(){
        ...
        if ($file['size'][0]==0){
            ...
            if($row==1){
                $this->jump('添加成功','index.php?m=admin&f=jewelry&a=init');
            }else{
                $this->jump('添加失败','index.php?m=admin&f=jewelry&a=init');
            }
            exit;
        }
        ...
        foreach ($brr as $key=>$item){
            if (!in_array($item,$type)){
                $this->jump('请上传图片','index.php?m=admin&f=jewelry&a=addcategory');
    ...
    function change(){
        ...
        $this->s->assign('id',$id);
        $this->s->display('template/admin/changejewelry.php');
    }
    function addtext(){
        $id=$_REQUEST['id'];
        $name=$_REQUEST['name'];
