...
class lifu extends admin
{
    ...
    {
        parent::__construct();
        $this->db=new db('dress');
    }
//    内容页面展示
    function init(){
        $this->getcontent($this->db);
        $this->s->assign('arr',$this->brr);
        $this->s->display('template/admin/lifu.html');
    }
                ...
                $this->brr.="
                  <tr>
                    ...
                    <td>
                        ...
                        <button type='button' class='btn btn-primary'><a href='index.php?m=admin&f=lifu&a=change&id={$item['id']}'>修改</a></button>
                    </td>
                 </tr>
                ";
            }
...
//    内容添加功能实现
    function addcontent(){
        ...
        $this->s->display('template/admin/lifuadd.php');
    }
    function getoption($db,$id=4){
    ...
    function addcheck(){
        ...
        if ($file['size'][0]==0){
            ...
            if($row==1){
                $this->jump('添加成功','index.php?m=admin&f=lifu&a=init');
            }else{
                $this->jump('添加失败','index.php?m=admin&f=lifu&a=init');
            }
            exit;
        }
        ...
        foreach ($brr as $key=>$item){
            if (!in_array($item,$type)){
                $this->jump('请上传图片','index.php?m=admin&f=lifu&a=addcategory');
    ...
    function change(){
        ...
        $this->s->assign('id',$id);
        $this->s->display('template/admin/changlifu.php');
    }
    function addtext(){
        $id=$_REQUEST['id'];
        $name=$_REQUEST['name'];
        ...
        if ($file['size'][0]==0){
            ...
            if($row==1){
                $this->jump('修改成功','index.php?m=admin&f=lifu&a=init');
            }else{
                $this->jump('修改失败','index.php?m=admin&f=lifu&a=init');
            }
            exit;
        }
        $type=['image/jpeg','image/png','image/gif'];
        ...
        foreach ($brr as $key=>$item){
            if (!in_array($item,$type)){
                $this->jump('请上传图片','index.php?m=index&f=lifu&a=addcategory');
                include_once "template/admin/tiaozhuan.html";
                exit();
            }
        }
