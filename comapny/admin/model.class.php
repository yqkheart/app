...
class model extends admin
{
    ...
    {
        parent::__construct();
        $this->db=new db('modeling');
    }
//    内容页面展示
    function init(){
        $this->getcontent($this->db);
        $this->s->assign('arr',$this->brr);
        $this->s->display('template/admin/model.html');
    }
...
//    内容添加功能实现
    function addcontent(){
        ...
        $this->s->display('template/admin/modeladd.php');
    }
    function getoption($db,$id=7){
    ...
    function addcheck(){
        ...
        if ($file['size'][0]==0){
            ...
            if($row==1){
                $this->jump('添加成功','index.php?m=admin&f=model&a=init');
            }else{
                $this->jump('添加失败','index.php?m=admin&f=model&a=init');
            }
            exit;
        }
        ...
        foreach ($brr as $key=>$item){
            if (!in_array($item,$type)){
                $this->jump('请上传图片','index.php?m=admin&f=model&a=addcategory');
    ...
    function change(){
        ...
        $this->s->assign('id',$id);
        $this->s->display('template/admin/changemodel.php');
    }
    function addtext(){
        $id=$_REQUEST['id'];
        $name=$_REQUEST['name'];
